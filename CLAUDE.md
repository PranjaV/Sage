...
- 2026-04-26 | A | Phase 4 cognitive layer in. B15 deterministic `cognitive_score.py` + 6 unit tests (3 golden transcripts hit ok/watch/attention bands cleanly). B16 `nodes/cognitive.analyze_session` runs synchronously inside `/session/end`: scores → gpt-5-mini summary (calm, non-clinical, JSON-out) → `write_analysis` → bridge `cognitive:ready` ping via new `/internal/analysis-ready`. B17 Cortex Analyst semantic model (`cortex_semantic.yaml`) + `upload_cortex_model.py` stage uploader + `cortex_analyst.ask` (Snowflake session-token auth, executes returned SQL, gpt-4.1-mini one-sentence summary) + `POST /caregiver/ask`. B18 `seed_history.py` writes 30 days of Eleanor sessions: 6-transcript pool (normal_a..clear_a) → real cognitive_score → curve drifts ~95→73 with a 62 dip at d-5. CP2 not yet rehearsed.
